...
export function ProposalForm({ taskId }: { taskId: number }) {
  ...
  return <form className="panel proposal-form" onSubmit={(event) => { event.preventDefault(); void run(async () => {
    ...
    setMessage(''); setSolution(''); notify('Предложение отправлено. Статус: «На рассмотрении».'); refresh();
  }); }}>
    ...
    <fieldset disabled={busy}><TeamSelect />
      <label className="field">Сообщение бизнесу<textarea required maxLength={10000} rows={3} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Почему вашей команде интересна эта задача?" /></label>
      <label className="field">Предлагаемое решение<textarea required maxLength={10000} rows={5} value={solution} onChange={(event) => setSolution(event.target.value)} placeholder="Опишите подход, этапы работы и ожидаемый результат" /></label>
