...
export default function App() {
  ...
  useEffect(() => {
    const handler = () => { setRoute(location.hash.slice(1) || 'tasks'); window.scrollTo(0, 0); };
  ...
  function navigate(next: string) { location.hash = next; }
  function changeRole(next: Role) { setRole(next); storePreference('sana-role', next); navigate('tasks'); }
  ...
  else if (page === 'new' && role === 'business') content = <TaskWizard key="new" />;
  ...
  return <Context.Provider value={{ role, teamId, setTeamId, teams: teams.data ?? [], questions: questions.data ?? [], navigate,
